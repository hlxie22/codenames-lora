...
def eval_codenames_subset_raw(
    ...
    *,
    ...
    spymaster = InTrainerGenerator(model, tokenizer, device, disable_adapter=False)
    guesser = guesser_override if guesser_override is not None else InTrainerGenerator(model, tokenizer, device, disable_adapter=True)
    ...
    try:
        for start in range(0, len(boards), bs):
            ...
            for b, best, meta in zip(batch, bests, metas):
                per_board.append(build_codenames_record(b, best, meta))
...
def eval_gsm8k_raw(
    ...
    *,
    ...
    gen = InTrainerGenerator(model, tokenizer, device)
    gen_cfg = _GenCfg(
        temperature=0.0,
        top_p=1.0,
        top_k=None,
...
class EpochEvalCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        subset_local = shard_list(self._fixed_codenames_boards, rank, world)
        ...
        gathered_records = all_gather_objects(list(ref_raw.get("per_board", []) or []))
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        try:
            ...
            if n_boards > 0:
                ...
                subset_local = shard_list(subset, rank, world)
                guesser_override = self._get_external_guesser()
        ...
        gathered_records = all_gather_objects(per_board_local)
        per_board: List[Dict[str, Any]] = []
        for part in gathered_records:
            per_board.extend(part or [])
        per_board.sort(key=lambda r: str(r.get("board_id", "")))

        gathered_reasoning = all_gather_objects(reasoning_local)
        reasoning_tokens: List[int] = []
        ...
        total_nll = all_reduce_sum_float(float(wt_total_nll), device)
        total_tokens = all_reduce_sum_int(int(wt_total_tokens), device)
        blocks_done = all_reduce_sum_int(int(wt_blocks_done), device)
        ...
        gsm_correct = all_reduce_sum_int(gsm_correct_local, device)
        gsm_total = all_reduce_sum_int(gsm_total_local, device)
        ...
        he_pass = all_reduce_sum_int(he_pass_local, device)
        he_total = all_reduce_sum_int(he_total_local, device)
